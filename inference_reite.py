...
def extract_frames_from_mp4(video_path, fps):
...
def inference(args, model, save_path_prefix, in_path, text_prompts):
    ...
    if os.path.isfile(in_path) and not args.image_level:
        frames_folder, frames_list, ext = extract_frames_from_mp4(in_path, fps=args.fps)
    ...
    for i in range(len(text_prompts)):
        ...
        if not args.image_level:
            ...
            clip = ImageSequenceClip(out_files_w_mask, fps=args.fps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fps', default=10, type=int, help='FPS for the output video')
